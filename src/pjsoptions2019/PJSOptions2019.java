...
public class PJSOptions2019 {
    ...
    */
        ...
        ArrayList<TGenericModel> arrayListOptions = new ArrayList<>();
        ...
        someStock = new Qunderlying('F', 101, .30, divYield);

        System.out.println("Modelo CRR:");
        for (int i=0;i<5;i++) {
            TBinomialCRR CRR=new TBinomialCRR('A', someStock, 'P', 100, 10, .10, 3, 1000);
            arrayListOptions.add(CRR);
        }

        //*****test thread directo
        worker = new Thread[arrayListOptions.size()];
        ...
        for (TGenericModel optionFromList : arrayListOptions) {
            System.out.println("Binomial CRR-Thread:" + Arrays.toString(optionFromList.getDerivativesArray()[0]));//+"Implied VLT.."+tOpt1.getImpliedVlt());

        }
        System.out.println("\nElapsed Time Total no Q Thread        :" + (System.currentTimeMillis() - startTime));


        //*** end test thread directo



        TaskConcurrentQueue tcQueue = new TaskConcurrentQueue();
        tcQueue.taskQueue.addAll(arrayListOptions);
        tcQueue.startQueueWorkers();
     //   arrayListOptions.clear();
